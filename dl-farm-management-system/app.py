...
def edit_staff(staff_id):
    ...
    if request.method == 'POST':
        ...
        try:
            cur.execute('UPDATE staff SET first_name=?, last_name=?, position=?, department=?, email=?, phone=?, status=? WHERE id=?',
...
def tasks():
    conn = get_db_connection(); cur = conn.cursor()
    try:
        cur.execute('SELECT * FROM task ORDER BY due_date'); tasks = cur.fetchall()
    except:
        tasks = []
    try:
        cur.execute("SELECT COUNT(*) FROM task WHERE status = 'Pending'"); pending_count = cur.fetchone()[0] or 0
    except:
        pending_count = 0
    try:
        cur.execute("SELECT COUNT(*) FROM task WHERE status = 'In Progress'"); inprogress_count = cur.fetchone()[0] or 0
    except:
        inprogress_count = 0
    try:
        cur.execute("SELECT COUNT(*) FROM task WHERE status = 'Completed'"); completed_count = cur.fetchone()[0] or 0
    except:
        completed_count = 0
    try:
        today_str = date.today().isoformat()
        ...
        overdue_count = 0
    conn.close()
    return render_template('tasks.html', tasks=tasks, pending_count=pending_count, inprogress_count=inprogress_count, completed_count=completed_count, overdue_count=overdue_count)
...
def add_task():
    ...
    try:
        ...
        if not title:
            flash('Task title is required.', 'error'); return redirect(url_for('tasks'))
        conn = get_db_connection(); cur = conn.cursor()
        cur.execute('INSERT INTO task (title, description, assigned_to, status, priority, due_date, category) VALUES (?, ?, ?, ?, ?, ?, ?)',
                    (title, request.form.get('description', ''), request.form.get('assigned_to', ''), 'Pending', request.form.get('priority', 'Normal'), request.form.get('due_date'), request.form.get('category', 'general')))
        conn.commit(); conn.close(); flash('Task added!', 'success')
    except Exception as e:
...
def update_task_status():
    try:
        ...
        conn = get_db_connection(); cur = conn.cursor()
        cur.execute('UPDATE task SET status = ? WHERE id = ?', (status, task_id)); conn.commit(); conn.close()
